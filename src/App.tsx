...
export default function App() {
  ...
  const compareLayout = useRef<HTMLDivElement>(null);
  ...
  return <div className="app">
    ...
    <main>
      ...
      {comparing && <><div className="compare-mobile-tabs"><button className={mobileCompareSide === 0 ? 'active' : ''} onClick={() => setMobileCompareSide(0)}>{stages[compareLeft - 1].title}</button><button className={mobileCompareSide === 1 ? 'active' : ''} onClick={() => setMobileCompareSide(1)}>{stages[compareRight - 1].title}</button></div><div ref={compareLayout} className="compare-layout" style={{ gridTemplateColumns: `minmax(0, ${compareWidth}fr) 10px minmax(0, ${100 - compareWidth}fr)` }}><div className={`compare-column${mobileCompareSide === 0 ? ' mobile-active' : ''}`}>{compareStage(compareLeft)}</div><div className="compare-divider" role="separator" aria-label="Resize comparison panes" aria-orientation="vertical" aria-valuemin={30} aria-valuemax={70} aria-valuenow={compareWidth} tabIndex={0} onPointerDown={event => { event.currentTarget.setPointerCapture(event.pointerId); event.preventDefault(); }} onPointerMove={event => { if (!event.currentTarget.hasPointerCapture(event.pointerId) || !compareLayout.current) return; const bounds = compareLayout.current.getBoundingClientRect(); setCompareWidth(Math.max(30, Math.min(70, Math.round((event.clientX - bounds.left) / bounds.width * 100)))); }} onKeyDown={event => { if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') { event.preventDefault(); setCompareWidth(value => Math.max(30, Math.min(70, value + (event.key === 'ArrowLeft' ? -5 : 5)))); } }} /><div className={`compare-column${mobileCompareSide === 1 ? ' mobile-active' : ''}`}>{compareStage(compareRight)}</div></div></>}
