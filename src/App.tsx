...
function InlineBlock({ block, draft, depth, activeId, activate, change, rename }: { block: DraftBlock; draft: ConversionDraft; depth: number; activeId: string | null; activate: (id: string) => void; change: (id: string, value: Partial<DraftBlock>) => void; rename: (id: string, label: string) => void }) {
  ...
  return <section className={`inline-block depth-${Math.min(depth, 3)}${activeId === block.id ? ' active' : ''}`}>
    <div className="inline-boundary"><button className="inline-fold" aria-label={`${folded ? 'Expand' : 'Fold'} ${block.title}`} onClick={() => { activate(block.id); setFolded(!folded); }}>{folded ? '▸' : '▾'}</button><details className="inline-meta"><summary onClick={() => activate(block.id)}><span className="inline-kind">{block.kind}</span><span className="inline-title">{block.title}</span><span className="inline-label" title={block.label}>{block.label}</span></summary><div className="inline-meta-fields"><label>Kind<select value={block.kind} onChange={event => change(block.id, { kind: event.target.value as BlockKind })}>{blockKinds.map(kind => <option key={kind} value={kind}>{kind}</option>)}</select></label><label>Title<input value={block.title} onChange={event => change(block.id, { title: event.target.value })} /></label><label>Label<input value={label} onChange={event => setLabel(event.target.value)} onBlur={() => rename(block.id, label)} onKeyDown={event => { if (event.key === 'Enter') event.currentTarget.blur(); }} /></label><small>ID: {block.id}</small></div></details></div>
