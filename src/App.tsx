...
export default function App() {
  ...
  function compareStage(number: number) {
    ...
    if (number === 4) return <section className="pane compare-pane"><div className="pane-head"><strong>Block files</strong><span>{draft.blocks.length} files · select a block</span></div><div className="all-files"><details className="settings-file"><summary>Settings · setting/settings.json</summary><pre>{settingsJson(draft)}</pre></details>{draft.blocks.map(block => <section className={`exported-block${selectedId === block.id ? ' selected' : ''}`} key={block.id} onClick={() => focusCompareBlock(block.id)}><div className="exported-block-head"><strong>{block.title}</strong><span>{block.kind} · {block.id}.md</span></div><div className="exported-label">{block.label}</div><pre>{blockMarkdown(block)}</pre></section>)}</div></section>;
  ...
  return <div className="app">
    ...
    <main>
      ...
      {!comparing && stage === 4 && <div className="panes"><section className="pane"><div className="pane-head"><strong>Block data</strong><span>{draft.blocks.length} files</span></div><div className="pane-content">{blockList}{blockEditor}</div></section><section className="pane"><div className="pane-head"><strong>Exported files</strong><span>All blocks in reading order</span></div><div className="all-files"><details className="settings-file"><summary>Settings · setting/settings.json</summary><pre>{settingsJson(draft)}</pre></details>{draft.blocks.map(block => <section className="exported-block" key={block.id}><div className="exported-block-head"><strong>{block.title}</strong><span>{block.kind} · {block.id}.md</span></div><div className="exported-label">{block.label}</div><pre>{blockMarkdown(block)}</pre></section>)}</div><div className="export-tree"><strong>Workspace ZIP</strong><span>{draft.blocks.length} block .md files</span><span>setting/settings.json</span>{draft.assets.map(file => <span key={file.path}>{assetOutputPath(file.path, draft.mainPath)}</span>)}</div></section></div>}
