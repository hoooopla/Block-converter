...
function InlineBlock({ block, draft, depth, change, rename }: { block: DraftBlock; draft: ConversionDraft; depth: number; change: (id: string, value: Partial<DraftBlock>) => void; rename: (id: string, label: string) => void }) {
  ...
  return <section className={`inline-block depth-${Math.min(depth, 3)}`}>
    <div className="inline-boundary"><button className="inline-fold" aria-label={`${folded ? 'Expand' : 'Fold'} ${block.title}`} onClick={() => setFolded(!folded)}>{folded ? '▸' : '▾'}</button><details className="inline-meta"><summary><span>{block.kind}</span> {block.title}</summary><div className="inline-meta-fields"><label>Kind<select value={block.kind} onChange={event => change(block.id, { kind: event.target.value as BlockKind })}>{blockKinds.map(kind => <option key={kind} value={kind}>{kind}</option>)}</select></label><label>Title<input value={block.title} onChange={event => change(block.id, { title: event.target.value })} /></label><label>Label<input value={label} onChange={event => setLabel(event.target.value)} onBlur={() => rename(block.id, label)} onKeyDown={event => { if (event.key === 'Enter') event.currentTarget.blur(); }} /></label><small>ID: {block.id}</small></div></details></div>
    {!folded && <><div className="inline-scope">{pieces.map((piece, index) => {
        const match = piece.match(/^\[\[([^\]\n]+)∨\]\]$/);
        if (match) {
          const child = children.find(candidate => candidate.label === match[1]);
          return child ? <InlineBlock key={child.id} block={child} draft={draft} depth={depth + 1} change={change} rename={rename} /> : <textarea key={index} className="inline-text" aria-label={`${block.title} Markdown`} value={piece.trim()} onChange={event => editPiece(index, event.target.value)} spellCheck={false} />;
        }
        return piece.trim() ? <textarea key={index} className="inline-text" aria-label={`${block.title} Markdown`} value={piece.replace(/^\n+|\n+$/g, '')} onChange={event => editPiece(index, event.target.value)} spellCheck={false} /> : null;
      })}{children.filter(child => !block.content.includes(`[[${child.label}∨]]`)).map(child => <InlineBlock key={child.id} block={child} draft={draft} depth={depth + 1} change={change} rename={rename} />)}</div><div className="inline-end">End {block.kind}</div></>}
  </section>;
}
